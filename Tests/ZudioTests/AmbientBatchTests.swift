...
struct AmbientBatchTests {
    ...
    @Test func generateAmbPno003Batch() throws {
        let dir = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Downloads/Zudio/tools/batch-output/ambient-pno003")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let fm = FileManager.default
        let existing = (try? fm.contentsOfDirectory(at: dir, includingPropertiesForKeys: nil)) ?? []
        ...
        for i in 1...10 {
            ...
            let seedHex  = String(format: "%016llx", seed)
            let filename = String(format: "pno003_%02d_%@.MID", i, seedHex)
            let midiURL  = dir.appendingPathComponent(filename)
            try MIDIFileExporter.export(song, to: midiURL)
            try SongLogExporter.export(song, midiURL: midiURL)
            let keyMode = (song.frame.key + " " + song.frame.mode.rawValue)
                .padding(toLength: 22, withPad: " ", startingAt: 0)
            print("  \(i). \(keyMode)  \(song.frame.tempo) BPM  \(song.frame.totalBars) bars  \(song.title)")
        }
        print("\n✓ Done.")
    }
}
