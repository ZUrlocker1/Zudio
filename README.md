...
**What's new in v2.6:**
- **Kraftwerk flavour in Motorik** — Some Motorik songs now lean machine-like rather than Neu!-like. A group of tracks **syncs**: they adopt Kraftwerk rules together — rigid sequencer cells locked to the bar, sparse mechanical drums with no mid-section fills, octave doubling between parts — while the rest of the song plays normally. Synced tracks run slower (120-132 BPM) and drop out together twice a song, with the texture track left playing through the gap. A synced song takes a German-flavoured title so you can spot one in the Songs list, and its generation log names the tracks on a `Sync` line.
