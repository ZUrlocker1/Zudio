    ...
    static func presentShare(song: SongState) {
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                ...
                // The wording covers both platforms deliberately: iOS offers
                // "Copy to Zudio" on the attachment, macOS offers "Open in Zudio", and the
                // sender has no idea which the recipient is on. The instruction is there
                // because neither affordance is obvious — iOS 26 removed the original one,
                // and it only came back by pairing an opaque file type with the
                // ZudioQuickLook preview extension. See the note in iOS-Info.plist.
                mc.body = "Here's a cool Zudio song. Click on the file then click on Copy or Open in Zudio."
