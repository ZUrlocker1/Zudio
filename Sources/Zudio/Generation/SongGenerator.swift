...
struct SongGenerator {
    ...
    private static func generateMotorik(
        ...
        // Kraftwerk sync — base Motorik only, never Noir or Arcade. A coupled group of
        ...
        // change is invisible to every song that does not draw a sync.
        var syncRNG = SeededRNG(seed: SeededRNG.trackSeed(globalSeed: seed, trackIndex: 64))
        ...
        let frame = MusicalFrameGenerator.generate(
            ...
            moodOverride: moodOverride,
            motorikNoir: isNoir,
            motorikArcade: isArcade, kraftwerkSync: motorikSync.isActive
        )
        ...
        let syncWindows = syncDropoutWindows(sync: motorikSync, totalBars: frame.totalBars, seed: seed)
        ...
        trackEvents[kTrackDrums]   = DrumGenerator.generate(frame: frame, structure: structure, rng: &drumRNG, usedRuleIDs: &drumRules, forceRuleID: forceDrumRuleID, noirVariation: isNoir, arcadeVariation: isArcade, sync: motorikSync)
        ...
        let (lead1Events, ld1SoloRange) = LeadGenerator.generateLead1(frame: frame, structure: structure, tonalMap: tonalMap, rng: &lead1RNG, usedRuleIDs: &lead1Rules, forceLeadRuleID: forceLeadRuleID, noirVariation: isNoir, arcadeVariation: isArcade, sync: motorikSync)
        trackEvents[kTrackLead1] = lead1Events

        // When Lead 1 picks a dense arp rule, bias Bass and Lead 2 toward sparse/pulse roles.
        let arcadeDenseArpRules: Set<String> = ["MOT-LD1-015","MOT-LD1-017","MOT-LD1-018","MOT-LD1-019"]
        let arcadeDenseMelody = isArcade && !lead1Rules.isDisjoint(with: arcadeDenseArpRules)

        // Step 6 — Bass
        var bassRules: Set<String> = []
        trackEvents[kTrackBass]    = BassGenerator.generate(frame: frame, structure: structure, tonalMap: tonalMap, rng: &bassRNG, usedRuleIDs: &bassRules, forceRuleID: forceBassRuleID, noirVariation: isNoir, arcadeVariation: isArcade, arcadeDenseMelody: arcadeDenseMelody, sync: motorikSync)

        // Step 7 — Pads
        var padRules: Set<String> = []
        trackEvents[kTrackPads]    = PadsGenerator.generate(frame: frame, structure: structure, tonalMap: tonalMap, rng: &padsRNG, usedRuleIDs: &padRules, noirVariation: isNoir, arcadeVariation: isArcade)

        // Step 8 — Lead 2
        // Noir always suppresses Lead 2 — skip generation entirely (lead2RNG is not consumed elsewhere)
        if !isNoir {
            trackEvents[kTrackLead2] = LeadGenerator.generateLead2(frame: frame, structure: structure, tonalMap: tonalMap, lead1Events: lead1Events, rng: &lead2RNG, usedRuleIDs: &lead2Rules, soloRange: ld1SoloRange, arcadeVariation: isArcade, arcadeDenseMelody: arcadeDenseMelody)
        }

        // Step 8 — Rhythm
        var rhythmRules: Set<String> = []
        trackEvents[kTrackRhythm]  = RhythmGenerator.generate(frame: frame, structure: structure, tonalMap: tonalMap, rng: &rhythmRNG, usedRuleIDs: &rhythmRules, forceRuleID: forceRhythmRuleID, noirVariation: isNoir, arcadeVariation: isArcade, sync: motorikSync)

        if motorikSync.includes(kTrackRhythm) {
        ...
        // Step 8b — Kraftwerk sync: Lead 2 is locked to Rhythm.
        //
        ...
        // sequencer is exactly the incoherence the sync design exists to prevent. Variety
        ...
        var syncLead2Partners = false
        if motorikSync.includes(kTrackRhythm) {
            var lead2SyncRNG = SeededRNG(seed: SeededRNG.trackSeed(globalSeed: seed, trackIndex: 67))
        ...
        trackEvents[kTrackTexture] = TextureGenerator.generate(frame: frame, structure: structure, tonalMap: tonalMap, rng: &texRNG, usedRuleIDs: &texRules, noirVariation: isNoir, arcadeVariation: isArcade, sync: motorikSync, syncWindows: syncWindows)
        ...
        trackEvents = ArrangementFilter.apply(trackEvents: trackEvents, frame: frame, structure: structure, seed: seed, lead1SoloRange: ld1SoloRange, arcadeVariation: isArcade, exemptTracks: motorikSync.isActive ? motorikSync.tracks + [kTrackLead2] : [])
        ...
        // A fill on the bar before the sync leaves, and on the bar before it returns.
        let syncSeamBars = syncWindows.flatMap { [$0.lowerBound - 1, $0.upperBound - 1] }
        trackEvents = DrumVariationEngine.apply(trackEvents: trackEvents, frame: frame, structure: structure, seed: seed, noirVariation: isNoir, arcadeVariation: isArcade, kraftwerkSync: motorikSync.isActive, syncBoundaryBars: syncSeamBars, syncBassIsLocked: motorikSync.includes(kTrackBass))
        ...
        // otherwise break up a long run is skipped for every sync song.
        let guardedTracks = motorikSync.isActive ? Array(0..<kTrackCount) : []
        ...
        let title: String = {
            ...
            // A Kraftwerk sync renames the song instead of taking the Noir/Arcade path —
            // those two never coexist with a sync anyway. Drawn on its own stream so adding
            // the affix does not shift the shared RNG and change the rest of the song.
            if motorikSync.isActive {
            ...
            if isArcade && (isSingleWord || words.contains(where: { $0.count <= 2 })) {
                let arcadeWords = ["Overdrive", "Blast", "Zap", "Warp", "1999", "Grid", "Vector", "System", "Circuit", "Level", "Zero", "2000", "4k", "Alien"]
                let prob: Double = raw.count < 12 ? 0.80 : 0.0
                if rng.nextDouble() < prob {
                    return "\(raw) \(arcadeWords[rng.nextInt(upperBound: arcadeWords.count)])"
                }
                return raw
            }
            guard isNoir && (isSingleWord || words.contains(where: { $0.count <= 2 })) else { return raw }
            let prob: Double = raw.count < 12 ? 0.75 : 0.0
            guard rng.nextDouble() < prob else { return raw }
        ...
        // Track suppression — drawn after title so frame/structure/title are unaffected
        if isNoir {
        ...
        // Arcade: 40% texture suppression (less than Noir's 50% — still has occasional atmospheric colour)
        if isArcade && rng.nextDouble() < 0.40 {
            trackEvents[kTrackTexture] = []
            texRules = ["MOT-TEX-000"]
        }
        ...
        let log = buildLog(
            ...
            sync: motorikSync, syncLead2Partners: syncLead2Partners
        )

        let stepAnnotations = buildStepAnnotations(structure: structure, trackEvents: trackEvents, frame: frame, drumRules: drumRules, soloRange: ld1SoloRange, soloRuleID: lead1Rules.sorted().first(where: { $0 == "MOT-LD1-007" || $0 == "MOT-LD1-008" }), kraftwerkSync: motorikSync.isActive)
    ...
    private static func generateKosmic(
        ...
        if isKosmicDrift {
            ...
            if drumRules.contains("KOS-DRUM-007") {
                ...
                drumDropoutConfig = nil   // KOS-DRUM-001 Minimal and KOS-DRUM-000 Absent: no additional dropout
            }
    ...
    private static func generateAmbient(
        ...
        let isAmbientPiano = forceAmbientPianoRule != nil || pianoRoll < 0.35
        ...
        if let audioFile = ambientAudioTexture {
            ...
            texRules.insert(Self.ambientAudioTextureRuleID(for: audioFile))
        } else {
        ...
        if lead1RNG.nextDouble() < xFilesProb {
            ...
            if bar30 <= bar60 {
                ...
                trackEvents[kTrackLead1] = (trackEvents[kTrackLead1] + whistleEvents)
                    .sorted { $0.stepIndex < $1.stepIndex }
                // AMB-XFILES-001 intentionally not logged — playback annotation "Bar N  Spooky X-Files theme" is sufficient
        ...
        if !trackEvents[kTrackPads].isEmpty {
            ...
            // Sort once after all void fills rather than after each insert
            if !voids.isEmpty {
    ...
    static func regenerateTrack(_ trackIndex: Int, songState: SongState, overrideSeed: UInt64? = nil, passBodyBars: Int? = nil) -> SongState {
        ...
        case kTrackDrums:
            if isAmbPiano {
                ...
                let rawDrum = DrumGenerator.generate(frame: songState.frame, structure: songState.structure, rng: &rng, usedRuleIDs: &usedRules, noirVariation: songState.motorikNoirVariation, arcadeVariation: songState.motorikArcadeVariation)
                var scratch = songState.trackEvents
                scratch[kTrackDrums] = rawDrum
                events = DrumVariationEngine.apply(trackEvents: scratch, frame: songState.frame, structure: songState.structure, seed: newTrackSeed, noirVariation: songState.motorikNoirVariation, arcadeVariation: songState.motorikArcadeVariation)[kTrackDrums]
            }
        case kTrackBass:
            if isAmbPiano {
                return songState
        ...
        case kTrackPads:
            if isAmbPiano {
                ...
                events = PadsGenerator.generate(frame: songState.frame, structure: songState.structure, tonalMap: songState.tonalMap, rng: &rng, usedRuleIDs: &usedRules, noirVariation: songState.motorikNoirVariation, arcadeVariation: songState.motorikArcadeVariation)
            }
        case kTrackLead1:
            if isAmbPiano {
                ...
                // lead1BaseRule and xFilesBars are needed during initial song generation
                // (Lead 2 and Arpeggio reference them) but are unused in single-track regen.
                var unusedBaseRule = ""
                var unusedXFilesBars: [Int] = []
                events = KosmicLeadGenerator.generateLead1(
                    frame: songState.frame, structure: songState.structure,
                    tonalMap: songState.tonalMap, rng: &rng, usedRuleIDs: &usedRules,
                    lead1BaseRule: &unusedBaseRule, xFilesBars: &unusedXFilesBars,
                    isKosmicDrift: songState.isKosmicDrift,
                    isDriftDreamscape: songState.isDriftDreamscape)
            } else if isChill {
                ...
                (events, _) = LeadGenerator.generateLead1(frame: songState.frame, structure: songState.structure, tonalMap: songState.tonalMap, rng: &rng, usedRuleIDs: &usedRules, passBodyBars: passBodyBars, noirVariation: songState.motorikNoirVariation, arcadeVariation: songState.motorikArcadeVariation)
            }
        case kTrackLead2:
            if isAmbPiano {
                return songState
                ...
                events = LeadGenerator.generateLead2(frame: songState.frame, structure: songState.structure, tonalMap: songState.tonalMap, lead1Events: songState.trackEvents[kTrackLead1], rng: &rng, usedRuleIDs: &usedRules, arcadeVariation: songState.motorikArcadeVariation)
            }
        case kTrackRhythm:
            if isAmbPiano {
                return songState
        ...
        case kTrackTexture:
            if isAmbPiano {
                ...
                if rng.nextDouble() < 0.45 {
                    ...
                    usedRules.insert(Self.ambientAudioTextureRuleID(for: file))
                } else {
                ...
                events = KosmicTextureGenerator.generate(
                    frame: songState.frame, structure: songState.structure,
                    tonalMap: songState.tonalMap,
                    kosmicProgFamily: songState.kosmicProgFamily,
                    rng: &rng, usedRuleIDs: &usedRules,
                    isKosmicDrift: songState.isKosmicDrift)
            } else if isChill {
                ...
                events = TextureGenerator.generate(frame: songState.frame, structure: songState.structure, tonalMap: songState.tonalMap, rng: &rng, usedRuleIDs: &usedRules, noirVariation: songState.motorikNoirVariation, arcadeVariation: songState.motorikArcadeVariation)
            }
        default:
            return songState
        }
    ...
    /// Maps an Ambient audio texture filename to its rule ID.
    /// Single source of truth — used during both initial generation and regenerateTrack().
    private static func ambientAudioTextureRuleID(for filename: String) -> String {
        switch filename {
        case "light_rain.m4a":       return "AMB-TEXT-003"
        case "rain-and-thunder.m4a": return "AMB-TEXT-005"
        case "ocean_waves.m4a":      return "AMB-TEXT-006"
    ...
    /// Builds the Chords log description: key + abbreviated mode + cleaned family label.
    /// Strips "Static " prefix and chord annotations (i ♭VII, sus4 etc.) from family labels.
    private static func chordsDescription(key: String, mode: Mode, familyLabel: String) -> String {
        let modeStr: String
        switch mode {
        case .MajorPentatonic: modeStr = "Maj Pentatonic"
        case .MinorPentatonic: modeStr = "Min Pentatonic"
        default:               modeStr = mode.rawValue
        }
        var label = familyLabel.hasPrefix("Static ") ? String(familyLabel.dropFirst(7)) : familyLabel
        for token in [" i ", " ♭", " sus4", " bVI", " bVII", " bII", " bIII", " bV"] {
            if let r = label.range(of: token) { label = String(label[..<r.lowerBound]); break }
        }
    ...
    /// The bars a sync leaves and returns. Computed separately from the dropout itself
    ...
    static func syncDropoutWindows(sync: MotorikSync, totalBars: Int, seed: UInt64) -> [Range<Int>] {
    ...
    /// Sync section dropout — the most distinctive measured feature in the corpus, and it
    /// spans tracks, so it belongs to the sync rather than to any single rule.
    ///
    ...
    /// event, not independent per-track rests, which is why this silences the whole sync at
    /// once. Tracks OUTSIDE the sync keep playing, and that is what makes the drop read as a
    ...
    /// Caps how many consecutive identical bars a sync track may play.
    ///
    ...
    private static func applySyncDropout(
    ...
    private static func buildLog(
        ...
        log.append(GenerationLogEntry(tag: "Chords", description: chordsDescription(key: frame.key, mode: frame.mode, familyLabel: progLabel)))
        ...
        if lead2Rules.contains("MOT-LD2-000") {
            ...
            for ruleID in lead2Rules.sorted() {
                if ruleID.hasPrefix("CHL-LD2-HARM-") || ruleID.hasPrefix("CHL-LD2-UNIS-") {
    ...
    private static func drumRuleDescription(_ ruleID: String) -> String {
        ...
        case "MOT-DRUM-000": return "No Drums"
        case "MOT-DRUM-009": return "Four on the Floor"
        ...
        case "KOS-DRUM-000": return "No percussion"
    ...
    private static func bassRuleDescription(_ ruleID: String) -> String {
        ...
        case "MOT-BASS-022": return "Chrome Walk"
    ...
    private static func lead1RuleDescription(_ ruleID: String) -> String {
        ...
        case "MOT-LD1-015": return "Arcade Riff"
        case "MOT-LD1-016": return "Machine Answer"
        case "MOT-LD1-017": return "Pentatonic Blitz"
    ...
    private static func lead2RuleDescription(_ ruleID: String) -> String {
        ...
        case "MOT-LD2-007": return "Arcade Arp"
    ...
    private static func kosmicPadRuleDescription(_ ruleID: String) -> String {
        ...
        case "KOS-PADS-006":  return "Electric Buddha shimmer"
    ...
    private static func kosmicRthmRuleDescription(_ ruleID: String) -> String {
        switch ruleID {
        case "KOS-RTHM-001": return "Stochastic TD sequencer"
    ...
    private static func kosmicProgressionFamilyLabel(_ family: KosmicProgressionFamily) -> String {
        ...
        case .suspended_resolution:  return "Sus Resolution"
    ...
    private static func buildKosmicLog(
        ...
        log.append(GenerationLogEntry(tag: "Style", description: "\(styleDesc) – \(frame.tempo) bpm"))
        ...
        log.append(GenerationLogEntry(tag: "Chords", description: chordsDescription(key: frame.key, mode: frame.mode, familyLabel: progFamilyLabel)))
    ...
    private static func buildAmbientLog(
        ...
        log.append(GenerationLogEntry(tag: "SONG", description: title, isTitle: true))
        log.append(GenerationLogEntry(tag: "Style", description: "\(isAmbientPiano ? "Ambient Piano" : "Ambient") – \(frame.tempo) bpm"))
    ...
    private static func ambientRuleDescription(_ ruleID: String) -> String {
        ...
        case "AMB-BASS-000":  return "No bass"
        case "AMB-BASS-003":  return "Root+fifth drone"
        case "AMB-BASS-004":  return "ECM pluck"
        case "AMB-BASS-005":  return "Octave double"
        case "AMB-BASS-006":  return "Slow pendulum"
        // Lead 1
        ...
        case "AMB-RTHM-000": return "No rhythm"
        ...
        case "AMB-TEXT-000": return "No texture"
        ...
        case "AMB-DRUM-000": return "No percussion"
        default:                   return ruleID
        }
    }
    ...
    static func buildStepAnnotations(
        ...
        func fillBeats(bar: Int) -> Int {
            ...
            // fill and the log filled with cascades that never played. For a sync, measure
        ...
        // A Kraftwerk sync fills only at its dropout edges, so the three sources above — which
        ...
        if includeDrumFills && kTrackDrums < trackEvents.count {
            ...
            var rideAnnounceCount   = 0   // suppress after 2 — rapid back-and-forth is noisy
            var hatBackAnnounceCount = 0
            for bar in 0..<totalBars {
                ...
                if mode != prevCymbalMode {
                    switch mode {
                    case "ride":
                        if !isRideGroove && rideAnnounceCount < 2 {
                    ...
                    case "hat":
                        if prevCymbalMode != nil && prevCymbalMode != "hat" && hatBackAnnounceCount < 2 {
                            fireBar(bar, tag: "Drums", desc: "back on hi-hat")
                            hatBackAnnounceCount += 1
                        }
                    default: break
                    }
                    prevCymbalMode = mode
            ...
            for section in structure.sections {
                ...
                if let prev = prevStepFP, !steps.isEmpty, !prev.isEmpty {
                    ...
                    if union > 0 && Double(common) / Double(union) < 0.55 {
                        fireBar(section.startBar, tag: patternTag, desc: "pattern changes")
                    }
                }
        ...
        // Suppress repeated identical annotations: allow at most 2 consecutive occurrences
        // of the same description with no other message between them. Any different description
        // at any intervening bar resets the streak. This reduces clutter from drum fills and
        // cymbal patterns that may repeat many times without adding new information.
        var streakDesc: String? = nil
        var streakCount = 0
        var filtered: [Int: [GenerationLogEntry]] = [:]
        for (step, entries) in out.sorted(by: { $0.key < $1.key }) {
            var kept: [GenerationLogEntry] = []
            for entry in entries {
                let desc = entry.description
                if desc == streakDesc {
                    streakCount += 1
                    if streakCount <= 2 { kept.append(entry) }
                    // 3rd+ consecutive identical: silently drop
                } else {
                    streakDesc = desc
                    streakCount = 1
                    kept.append(entry)
            ...
            if !kept.isEmpty { filtered[step] = kept }
        }
        return filtered
    }
    ...
    private static func generateChill(
        ...
        var harmonyStepAnnotation:  (bar: Int, isUnison: Bool, intervalSteps: Int)? = nil
        var finaleStepAnnotation:   (bar: Int, isUnison: Bool, intervalSteps: Int)? = nil

        // Regular Chill harmony — 20% of non-blues songs get a second horn joining Lead 1
        // briefly in section A and (if present) section B. Same applyBluesHarmony function,
        // different window selection: ~40% into section A, ~30% into section B, 4-bar spans.
        if !bluesVariation && lead2RNG.nextDouble() < 0.20 {
            let useUnison     = lead2RNG.nextDouble() < 0.25
            let intervalSteps = lead2RNG.nextDouble() < 0.70 ? 2 : 5

            func applyRegularHarmonyWindow(_ sec: SongSection, annotSlot: inout (bar: Int, isUnison: Bool, intervalSteps: Int)?,
                                           minOffsetFraction: Double) {
                let winSize = 6   // wider window catches more Lead 1 phrases
                guard sec.lengthBars >= winSize + 4 else { return }

                // Find the 6-bar window with the most Lead 1 events — avoids landing in sparse stretches
                let scanFrom = sec.startBar + max(4, Int(Double(sec.lengthBars) * minOffsetFraction))
                let scanTo   = sec.endBar - winSize - 2
                guard scanTo > scanFrom else { return }
                var bestStart = scanFrom
                var bestCount = 0
                for start in scanFrom...scanTo {
                    let n = lead1Events.filter {
                        let b = $0.stepIndex / 16; return b >= start && b < start + winSize
                    }.count
                    if n > bestCount { bestCount = n; bestStart = start }
                }
                guard bestCount > 0 else { return }   // no Lead 1 activity in any window

                let harmSet = Set(bestStart..<bestStart + winSize)
                let beforeCount = lead2EventsFinal.filter({ harmSet.contains($0.stepIndex / 16) }).count
                ChillLeadGenerator.applyBluesHarmony(
                    lead1Events: lead1Events, lead2Events: &lead2EventsFinal,
                    harmonyBars: harmSet, useUnison: useUnison,
                    intervalSteps: intervalSteps, maxPhrases: 4,   // allow up to 4 phrases
                    frame: frame, rng: &lead2RNG
                )
                // Only log and annotate if harmony notes were actually added
                let harmonyNotes = lead2EventsFinal.filter({ harmSet.contains($0.stepIndex / 16) })
                guard harmonyNotes.count > beforeCount else { return }

                let tag = useUnison ? "CHL-LD2-UNIS-\(bestStart + 1)" : "CHL-LD2-HARM-\(bestStart + 1)"
                lead2Rules.insert(tag)
                if annotSlot == nil,
                   let first = harmonyNotes.min(by: { $0.stepIndex < $1.stepIndex }) {
                    annotSlot = (first.stepIndex / 16, useUnison, intervalSteps)
                }
            }

            if let secA = structure.sections.first(where: { $0.label == .A }) {
                applyRegularHarmonyWindow(secA, annotSlot: &harmonyStepAnnotation, minOffsetFraction: 0.30)
            }
            if let secB = structure.sections.first(where: { $0.label == .B }) {
                applyRegularHarmonyWindow(secB, annotSlot: &finaleStepAnnotation, minOffsetFraction: 0.20)
            }
        }

        // Blues harmony/unison sections — 25% of blues songs.
        ...
        if bluesVariation {
            ...
            if harmonyEnd <= outroStart && lead2RNG.nextDouble() < harmonyProb {
                ...
                if let firstNote = lead2EventsFinal.filter({ harmonySet.contains($0.stepIndex / 16) })
                    ...
                    harmonyStepAnnotation = (firstNote.stepIndex / 16, useUnison, intervalSteps)
                    // Tag for Form "- harmony" display — only when notes actually fired
                    lead2Rules.insert(useUnison ? "CHL-LD2-UNIS-\(harmonyStart + 1)" : "CHL-LD2-HARM-\(harmonyStart + 1)")
                }
                ...
                if numPasses >= 3 {
                    ...
                    if let firstNote = lead2EventsFinal.filter({ finaleSet.contains($0.stepIndex / 16) })
                                                        .min(by: { $0.stepIndex < $1.stepIndex }) {
                        finaleStepAnnotation = (firstNote.stepIndex / 16, useUnison, intervalSteps)
                    }
                }
            }
        }
        ...
        func harmonyDesc(_ h: (bar: Int, isUnison: Bool, intervalSteps: Int)) -> String {
            ...
            return "Lead 2 harmony - sixth"
        }
        if let ha = harmonyStepAnnotation {
            stepAnnotations[ha.bar * 16, default: []].append(GenerationLogEntry(tag: "", description: harmonyDesc(ha)))
        }
        if let fa = finaleStepAnnotation {
            stepAnnotations[fa.bar * 16, default: []].append(GenerationLogEntry(tag: "", description: harmonyDesc(fa)))
        }
    ...
    private static func buildChillLog(
        ...
        var log: [GenerationLogEntry] = []
        log.append(GenerationLogEntry(tag: "SONG",  description: title, isTitle: true))
        log.append(GenerationLogEntry(tag: "Style", description: "\(bluesVariation ? "Chill Blues" : "Chill") – \(frame.tempo) bpm"))
        ...
        let hasHarmony = lead2Rules.contains { $0.hasPrefix("CHL-LD2-HARM-") || $0.hasPrefix("CHL-LD2-UNIS-") }
        let harmonyTag = hasHarmony ? " - harmony" : ""
        let formDesc: String
        if bluesVariation {
            formDesc = "16-bar blues I-IV-V\(harmonyTag)"
        } else {
            formDesc = hasBreakdown ? "Groove - breakdown\(harmonyTag)" : "Groove\(harmonyTag)"
        }
        log.append(GenerationLogEntry(tag: "Form", description: formDesc))
        ...
        log.append(GenerationLogEntry(tag: "Chords", description: chordsDescription(key: frame.key, mode: frame.mode, familyLabel: familyLabel)))

        // Audio texture
        if let texName = chillAudioTexture {
            let textureDesc = texName.replacingOccurrences(of: ".m4a", with: "").replacingOccurrences(of: "_", with: " ")
            log.append(GenerationLogEntry(tag: "Texture", description: textureDesc))
        } else {
            log.append(GenerationLogEntry(tag: "CHL-TEXT-000", description: "No texture"))
        }
        ...
        let displayRules = drumRules.union(bassRules).union(padRules)
                            ...
                            .filter { !$0.hasPrefix("CHL-BLUES-IVV-")
                                   && !$0.hasPrefix("CHL-LD2-HARM-")   // shown at playback via step annotations
                                   && !$0.hasPrefix("CHL-LD2-UNIS-") }
        for ruleID in displayRules.sorted() {
            if ruleID.hasPrefix("CHL-LD2-HARM-") || ruleID.hasPrefix("CHL-LD2-UNIS-") {
                let barStr = ruleID.components(separatedBy: "-").last ?? "?"
    ...
    private static func chillRuleDescription(_ ruleID: String) -> String {
        ...
        case "CHL-DRUM-006": return "Blues side-stick"
        ...
        case "CHL-PAD-004":  return "Open fifth hold"
        case "CHL-PAD-005":  return "Slow chord build"
        case "CHL-PAD-006":  return "String hold"
        case "CHL-PAD-000":  return "No pads"
        ...
        case "CHL-RHY-001":  return "St Germain beat"
