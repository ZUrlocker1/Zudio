...
## V 2.5 (Build 130)  Endless mix rebalanced
...
- **Song sharing on iOS 26** — iOS 26 removed the "Open in Zudio" button that used to appear on a shared song. Not a Zudio bug — a plain `.txt` behaves identically, and the same build still works on iOS 18 and on Mac. Seven file-type variants were tested and ruled out, as were Mail and AirDrop. Three changes were made to work around it, and they help but do not fully solve it:
    - **A Share Extension** (`ZudioShare`). iOS 26 no longer routes an attachment to its owning app, and the only remaining way in is the share sheet, which is populated by extensions rather than by file-type registration. The extension has no UI: it copies the song into an App Group container shared with the app, then launches Zudio via a new `zudio://` URL scheme. Since an extension cannot reliably launch its own app, Zudio also checks that container whenever it becomes active.
    - **A non-text file type on iOS.** While `.zudio` conformed to a text type, iOS resolved it to "Text Document" and previewed it inline, and the share arrow never appeared. It now declares an opaque type, so it identifies as "Zudio Song" and can be shared into the app. The trade is no inline text preview on iPhone or iPad. Mac is deliberately left text-conforming, so Quick Look still previews as text and Open With still offers TextEdit.
    - **Instructions in the shared message**, since the share arrow is real but almost impossible to find unaided. Sending a song from iPhone now includes a one-line hint.
    Result: songs open reliably from Files and from any app that can share a file. Opening one directly from Messages is not reliable — tapping the attachment sometimes offers a preview with a share arrow that reaches Zudio, and sometimes does nothing, with no change in the build. That inconsistency is Apple's and cannot be fixed from here, so the shared message wording is deliberately hedged. Filed with Apple as FB24777501.
- **Mac file type fixed** — `.zudio` files were showing as text documents and opening the iPad app instead of the Mac app. Caused by stale Launch Services registrations from old iPad-on-Mac builds; clearing them restored the correct kind and handler.
...
- **Possible next steps for iOS sharing** — Not yet decided:
    - File Feedback Assistant with the plain `.txt` reproduction, which is a far stronger report than a custom file type.
    - Wait for an iOS point release. The bug affects every text attachment, not a niche case, so a fix is plausible.
    - Share songs as a `zudio://` link instead of a file. Links are tappable in Messages on every iOS version, and a song is fully reproducible from its seed and overrides — the `.zudio` file already carries nothing more than that.
