...
- **"Open in Zudio" missing in Messages on iOS 26** — Investigated and traced to a change in iOS 26, not to Zudio. On iOS 18 a `.zudio` attachment previews as text *and* shows an "Open in Zudio" chip in the Quick Look toolbar; on iOS 26 that chip is gone, and the chevron menu offers only Save to Files and Print. Confirmed by running the identical 2.4 build on an iOS 18 device (chip present) and an iOS 26.2 device (chip absent) — the OS version is the only variable. Ruled out along the way: the file is typed correctly as `com.zudio.song` on both (verified in the Messages database), so this is not a file-type problem; and text preview and the chip coexist happily on iOS 18, so the `public.plain-text` conformance is not the cause either. A MIME type declaration was tried and reverted, having made no difference. The remaining route into an app from a Messages attachment on iOS 26 is the share sheet, which is driven by Share Extensions — Zudio has none, which is why it doesn't appear there. Adding one is under consideration. **Workaround:** Save to Files, then tap the file, which still opens it in Zudio.
- **Mac file type fixed** — On Mac, `.zudio` files were showing as text documents and offering "Open in Zudio iOS", which launched the iPad app instead of the Mac app. Cause was stale Launch Services registrations left behind by old iPad-on-Mac builds sharing the `com.zudio.app` bundle ID (shared deliberately, for Universal purchase). Clearing them restored the correct document kind and handler.
