...
- **Texture speaks in the gaps** — When a synchronised Motorik group drops out, Texture now plays through it instead of dropping out with the rest, so the gap reads as a change of texture rather than a hole. The Sparse Punctuation rule also places its bursts against those gaps, not only at section boundaries: a Motorik song has about three sections, so it previously managed two bursts and six notes across a whole song. It now averages ten notes in four or five bursts — still very sparse, about one burst every twenty-five bars.
- **More Kraftwerk flavour in Motorik** — About one Motorik song in seven now leans more Kraftwerk machine-like rather than Neu!-like. Rather than a new substyle, a synchronized set of tracks adopts Kraftwerk rules together while the rest of the song draws from normal rotation. Twelve new Kraftwerk derived rules across drums, bass, rhythm, both leads and texture: rigid single-pitch percussion with few fills and a step sequencer's accent pattern rather than a drummer's swing, tiny sequencer cells locked to the barline, octave-unison doubling, and leads that make short statements and then stop. Synced tracks also drop out together twice a song while everything else keeps playing, and draw from a restricted set of the existing electronic instruments. Synced songs also run slower — 120-132 BPM against Motorik's usual 126-154, matching the tempo Kraftwerk actually worked at — and the drums fill only where the whole group drops out and returns. 
